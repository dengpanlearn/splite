...
void CTrainWidget::OnSelectInDir(bool checked)
{
	...
	QString selectDirPath = QFileDialog::getExistingDirectory(this, pTextCodec->toUnicode("Ñ¡Ôñ´ýÑµÁ·µÄÍ¼Æ¬Â·¾¶"), defaultDir, QFileDialog::ShowDirsOnly
...
void CTrainWidget::OnSelectOutFile(bool checked)
{
	...
	defaultSaveFile = QFileDialog::getSaveFileName(this, pTextCodec->toUnicode("Ñ¡Ôñ´ýÑµÁ·µÄÍ¼Æ¬Â·¾¶"), defaultSaveDir, "xml (*.xml)");
	if (!defaultSaveFile.isEmpty())
...
void CTrainWidget::OnStartTrain(bool checked)
{
	...
	else
	{
		...
		msgBox.setWindowTitle(pTextCodec->toUnicode("ÌáÊ¾"));
		msgBox.setText(pTextCodec->toUnicode("ÑµÁ·Ê§°Ü"));
...
void CTrainWidget::OnTimeoutTrain()
{
	...
	if (g_trainTask.IsTriggerStop())
	{
		...
		msgBox.setWindowTitle(pTextCodec->toUnicode("ÌáÊ¾"));
		msgBox.setText(pTextCodec->toUnicode("ÑµÁ·Íê³É"));
